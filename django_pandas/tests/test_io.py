...
class IOTest(TestCase):
    ...
    def test_page(self):
        ...
        self.assertEqual(
            list(df.columns), ["id", "index_col", "col1", "col2", "col3", "col4"]
        )
    ...
    def test_choices(self):
        ...
        self.assertEqual(df.col1[0], "First \U0001f947")
        self.assertEqual(df.col1[1], "Second \U0001f948")
        self.assertEqual(df.col1[2], "Third \U0001f949")
        self.assertEqual(df.col1[3], "Second \U0001f948")
    ...
    def test_index(self):
        qs = MyModel.objects.all()
        df = read_frame(qs, ["col1", "col2", "col3", "col4"], index_col="index_col")
        self.assertEqual(df.shape, (qs.count(), 4))
        self.assertEqual(
            set(df.index.tolist()), set(qs.values_list("index_col", flat=True))
        )
...
class RelatedFieldsTest(TestCase):
    ...
    def test_verbose(self):
        ...
        self.assertListEqual(
            list(qs.values_list("trader__name", flat=True)), df.trader.tolist()
        )
        df1 = read_frame(qs, verbose=False)
        self.assertListEqual(
            list(qs.values_list("trader__pk", flat=True)), df1.trader.tolist()
        )
        ...
        if django.VERSION >= (1, 10):
            ...
            df2 = read_frame(qs1, fieldnames=["trader", "total_sum"])
            self.assertListEqual(
                list(qs1.values_list("total_sum", flat=True)), df2.total_sum.tolist()
            )
            self.assertListEqual(
                list(qs1.values_list("trader__name", flat=True)), df2.trader.tolist()
            )
    ...
    def test_verbose_duplicates_fieldnames(self):
        ...
        self.assertListEqual(
            list(qs.values_list("price", flat=True)), df.price.tolist()
        )
    ...
    def test_related_cols(self):
        ...
        self.assertListEqual(
            list(qs.values_list("symbol__isin", flat=True)), df.symbol__isin.tolist()
        )
        self.assertListEqual(
            list(qs.values_list("trader__name", flat=True)), df.trader__name.tolist()
        )
    ...
    def test_many_to_many(self):
        ...
        for idx, row in enumerate(denormalized):
            self.assertListEqual(df.iloc[idx].tolist(), list(row))
