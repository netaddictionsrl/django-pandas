...
class TimeSeriesTest(TestCase):
    ...
    def setUp(self):
        ...
        create_list = [
            LongTimeSeries(date_ix=timestamp, series_name=s.iloc[0], value=s.iloc[1])
    ...
    def test_longstorage(self):
        ...
        df = qs.to_timeseries(
            index="date_ix", pivot_columns="series_name", values="value", storage="long"
        )
        self.assertEqual(
            set(qs.values_list("series_name", flat=True)), set(df.columns.tolist())
        )
    ...
    def test_resampling(self):
        ...
        self.assertEqual(
            [d.month for d in qs.dates("date_ix", "month")], df.index.month.tolist()
        )
    ...
    def test_bad_args_wide_ts(self):
        ...
        kwargs = {"fieldnames": ["col1", "col2"], "freq": "M", "rs_kwargs": rs_kwargs}
    ...
    def setUp(self):
        ...
        create_list = [
            PivotData(
                ...
                row_col_b=r.iloc[1],
                row_col_c=r.iloc[2],
                value_col_d=r.iloc[3],
                value_col_e=r.iloc[4],
                value_col_f=r.iloc[5],
            )
...
if django.VERSION < (1, 9):
    ...
    class CreatePassThroughManagerTests(TestCase):
        ...
        def test_reverse_manager(self):
            Spot.objects.create(
                name="The Crib", owner=self.dude, closed=True, secure=True, secret=False
            )
            self.assertEqual(self.dude.spots_owned.closed().count(), 1)
            Spot.objects.create(
                name="The Crux",
                owner=self.other_dude,
                closed=True,
                secure=True,
                secret=False,
            )
